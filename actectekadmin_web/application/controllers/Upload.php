...
class Upload extends CI_Controller
{
	...
	public function pushFIle(){
		...
		echo json_encode(array('code' => 200,'src' => "https://ack.dltqwy.com".$src, 'msg' => "上传成功"));
		return;
	}
	/**
	 * pdf上传
	...
	public function pushFIletextarea(){
		...
		$data = array();
		$data['src'] = "https://ack.dltqwy.com".$src;
	...
	public function pushFIles(){
		...
		for ($i=0;$i<$count;$i++) {
			...
			if (file_exists("./static/upload/".$fileName)) {
				$src[]="https://ack.dltqwy.com"."/static/upload/".$fileName;
			}
		}
		$data = array();
		$data['src'] = $src;
	...
	public function base64_image_content($base64_image_content){
		...
		if (preg_match('/^(data:\s*image\/(\w+);base64,)/', $base64_image_content, $result)){
			...
			if (file_put_contents($new_file, base64_decode(str_replace($result[1], '', $base64_image_content)))){
				...
				return "https://ack.dltqwy.com".$src;
			}else{
				return false;
			}
		}else{
